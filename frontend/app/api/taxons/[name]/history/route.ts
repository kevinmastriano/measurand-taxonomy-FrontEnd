...
export async function GET(
  ...
  try {
    ...
    if (staticCache) {
      console.log('[TaxonHistory] Using static history cache from NCSLI-MII/measurand-taxonomy');
      cachedHistory = staticCache;
      isStatic = true;
    } else if (shouldUseStaticCache()) {
      console.log('[TaxonHistory] Static cache not found in production');
      cachedHistory = null;
    } else {
      // In development, fall back to dynamic cache only if static cache unavailable
      console.log('[TaxonHistory] Static cache not found, falling back to dynamic cache');
